...
df["Payment Method"].value_counts().plot(kind ="pie" , autopct="%1.1f%%", colors=colors, explode=(0.05,0 , 0));
df.Region.unique()
df.Region.value_counts()
colors = plt.get_cmap("Pastel1_r").colors
df["Region"].value_counts().plot(kind ="pie" , autopct="%1.1f%%", colors=colors, explode=(0,0 , 0),startangle=90)
sales_by_region = df.groupby("Region")["Units Sold"].sum().reset_index()
print(sales_by_region)

sns.barplot(data=sales_by_region, x="Region", y="Units Sold")

total_sales_by_category = df.groupby("Product Category")["Units Sold"].sum().reset_index()
print(total_sales_by_category)
plt.figure(figsize=(10,5))
sns.barplot(data=total_sales_by_category, x="Product Category", y="Units Sold")

total_sales_Payment_Method = df.groupby("Payment Method")["Units Sold"].sum().reset_index()
...
Product_Units =data.groupby("Product Name")["Units Sold"].sum().sort_values(ascending=False).reset_index()

data[data["Units Sold"].isin(Product_Units["Units Sold"].nlargest(8))]
data = df[df["Product Category"]== "Clothing"]
Product_Units = data.groupby("Product Name")["Units Sold"].sum().sort_values(ascending=False).reset_index()
data[data["Units Sold"].isin(Product_Units["Units Sold"].nlargest(3))].sort_values(by="Units Sold" , ascending=False)
plt.figure(figsize=(10,5))
...
sns.barplot(data=df_grouped, x="day_name", y="Units Sold", hue="Payment Method",palette=["#F1DEC6" , "#FFD7C4","#FFDBB5"],edgecolor="black")
...
plt.figure(figsize=(12, 6))
sns.barplot(data=df_grouped, x="month", y="Units Sold", hue="Payment Method",palette=["#F1DEC6" , "#FFD7C4","#FFDBB5"],edgecolor="black")
...
df_grouped=df.groupby(["Product Category","Region"])["Total Revenue"].sum().reset_index()
df_grouped=df.groupby(["Region","Product Category"])["Total Revenue"].sum().reset_index()
df_grouped.set_index(["Region", "Product Category"], inplace=True)
df_grouped = df.groupby(["Payment Method" ,"Product Category"])["Total Revenue"].sum().reset_index()
df_grouped.set_index(["Payment Method", "Product Category"], inplace=True)
df_grouped = df.groupby(["Payment Method" ,"Region"])["Total Revenue"].sum().reset_index()
df_grouped.set_index(["Payment Method", "Region"], inplace=True)
df_grouped =df.groupby(["Payment Method" ,"Region","Product Category"])["Units Sold"].sum().reset_index()
df_grouped.set_index(["Payment Method", "Region"], inplace=True)
df_grouped = df.groupby(["Payment Method", "Region", "Product Category"])["Total Revenue"].sum().reset_index()
df_grouped.set_index(["Payment Method", "Region", "Product Category"], inplace=True)
df_grouped = df.groupby(["Payment Method" ,"day_name"])["Total Revenue"].sum().reset_index()
df_grouped.set_index(["Payment Method", "day_name"], inplace=True)
df_grouped = df.groupby(["Payment Method" ,"month"])["Total Revenue"].sum().reset_index()
df_grouped.set_index(["Payment Method", "month"], inplace=True)
df["Product Category"].unique()

revenue_summary = df.groupby("Product Category")["Total Revenue"].agg(["min", "max"]).reset_index()
revenue_summary = df.groupby(["Payment Method","Product Category"])["Total Revenue"].agg(["min", "max"]).reset_index()
